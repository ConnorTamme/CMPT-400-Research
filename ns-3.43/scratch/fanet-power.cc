...
class RoutingExperiment
{
  ...
  private:
    ...
    std::string m_CSVSpecfileName{"manet-routing.output-spec.csv"}; //!< CSV filename.
    int m_nSinks{10};                                      //!< Number of sink nodes.
    int m_nWifis{50};                                       //!< Number of wifis in the network
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("CSVSpecfileName", "The name of the CSV output file name", m_CSVSpecfileName);
    ...
    cmd.AddValue("nWifis", "Number of Nodes", m_nWifis);
    cmd.AddValue("NewCSV", "Whether or not to remake the csv", m_NewCSV);
    cmd.AddValue("TotalTime", "Total time simulation runs", m_TotalTime);
    cmd.AddValue("MaxX", "Max X range", m_maxX);
    cmd.AddValue("MaxY", "Max Y range", m_maxY);
...
RoutingExperiment::Run()
{
    ...
    EnergySourceContainer energySourceContainer =
        batteryHelper.Install(adhocNodes, PANASONIC_CGR18650DA_LION);
    ...
    radioEnergyHelper.Set("TxCurrentA", DoubleValue(15)); //in amps
    radioEnergyHelper.Set("RxCurrentA", DoubleValue(10));
    radioEnergyHelper.Set("IdleCurrentA", DoubleValue(10));
    DeviceEnergyModelContainer deviceModels =  radioEnergyHelper.Install(adhocDevices, energySourceContainer);
   ...
   MobilityHelper mobilityAdhoc;
    ...
    ssSpeed << "ns3::UniformRandomVariable[Min=5.0|Max=" << nodeSpeed << "]";
    //ssSpeed << "ns3::UniformRandomVariable[Min=1.0|Max=1.0]";
    ...
    onoff1.SetAttribute("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1.0]")); //Always on
    ...
    for (int i = 0; i < m_nWifis; i++){
        ...
        if (i > m_nSinks*2){
         ...
         //        << "| Node " << i << ": " << 1-(batteryX->GetRemainingEnergy()/batteryX->GetInitialEnergy()) << " ");
         //NS_LOG_UNCOND(" *Initial Energy * "
           //      << "| Node " << i << ": " << batteryX->GetInitialEnergy() << " ");
